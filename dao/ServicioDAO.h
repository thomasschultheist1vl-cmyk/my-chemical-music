...
public:
    ...
    QSqlQuery listarQt(const QString &rol = "Supervisor", int idUsuario = 0) const { QSqlQuery q(conexionQt); QString sql = "SELECT s.id_servicio, CONCAT(c.nombre, ' ', c.apellido), s.instrumento, s.descripcion, DATE_FORMAT(s.fecha_ingreso, '%d/%m/%Y'), DATE_FORMAT(s.fecha_entrega, '%d/%m/%Y'), s.precio, es.nombre, COALESCE(s.motivo_anulacion, ''), COALESCE(DATE_FORMAT(s.fecha_anulacion, '%d/%m/%Y %H:%i'), ''), COALESCE(CONCAT(u.nombre, ' ', u.apellido, ' — ', r.nombre), 'Sin usuario') FROM servicios s LEFT JOIN clientes c ON s.id_cliente=c.id_cliente LEFT JOIN estados_servicio es ON s.id_estado_servicio=es.id_estado_servicio LEFT JOIN usuarios u ON s.id_usuario=u.id_usuario LEFT JOIN roles r ON u.id_rol=r.id_rol "; if (rol == "Vendedor") sql += "WHERE s.id_usuario=? OR r.nombre='Supervisor' "; sql += "ORDER BY s.id_servicio DESC"; q.prepare(sql); if (rol == "Vendedor") q.addBindValue(idUsuario); q.exec(); return q; }
    ...
    bool agregarQt(Servicio &s, int idUsuario = 0) const { QSqlQuery q(conexionQt); q.prepare("INSERT INTO servicios (id_cliente, instrumento, descripcion, fecha_ingreso, fecha_entrega, precio, id_estado_servicio, id_usuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"); q.addBindValue(s.getIdCliente()); q.addBindValue(QString::fromStdString(s.getInstrumento())); q.addBindValue(QString::fromStdString(s.getDescripcion())); q.addBindValue(QString::fromStdString(s.getFechaIngreso())); q.addBindValue(QString::fromStdString(s.getFechaEntrega())); q.addBindValue(s.getPrecio()); q.addBindValue(s.getIdEstadoServicio()); q.addBindValue(idUsuario > 0 ? QVariant(idUsuario) : QVariant()); return q.exec(); }
    ...
    QSqlQuery disponiblesParaFacturarQt(const QString &rol = "Supervisor", int idUsuario = 0) const {
        ...
        QString sql =
            ...
            + QString(rol == "Vendedor" ? "AND (s.id_usuario=? OR r.nombre='Supervisor') " : "") +
    ...
    bool anularServicioQt(int id, const QString &motivo, int idUsuarioAnulacion = 0, QString *error = nullptr) {
        ...
        update.prepare("UPDATE servicios SET id_estado_servicio=?, motivo_anulacion=?, fecha_anulacion=NOW(), id_usuario_anulacion=? WHERE id_servicio=?");
    ...
    bool perteneceAUsuarioOSupervisorQt(int id, int idUsuario) const { QSqlQuery q(conexionQt); q.prepare("SELECT COUNT(*) FROM servicios s LEFT JOIN usuarios u ON s.id_usuario=u.id_usuario LEFT JOIN roles r ON u.id_rol=r.id_rol WHERE s.id_servicio=? AND (s.id_usuario=? OR r.nombre='Supervisor')"); q.addBindValue(id); q.addBindValue(idUsuario); return q.exec() && q.next() && q.value(0).toInt() > 0; }
